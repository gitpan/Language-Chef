...
sub require_bowl {
   ...
   while (@{$self->{bowls}} < $no) {
      push @{$self->{bowls}}, Language::Chef::Container->new();
   }
...
sub require_dish {
   ...
   while (@{$self->{dishes}} < $no) {
      push @{$self->{dishes}}, Language::Chef::Container->new();
   }
...
sub compile {
   ...
   foreach (@ingredients) {
      ...
      my $measure = '';
      foreach my $meas ( keys %Language::Chef::Ingredient::Measures ) {
         next if $meas eq '';
      ...
      my $ingredient = Language::Chef::Ingredient->new(
...
Language::Chef::Recipe - Internal module used by Language::Chef
