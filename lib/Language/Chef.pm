...
# string is returned that reconstructs the Language::Chef
# object.
...
sub _paragraphsToRecipes {
   ...
   while (@$paragraphs) {
      ...
      push @recipes, Language::Chef::Recipe->new(
        name         => $recipe_name,
        comments     => $comments,
...
Language::Chef - An interpreter for the Chef programming language
...
=head1 SYNOPSIS
  ...
  my $compiled = Language::Chef->compile($code_string);  
